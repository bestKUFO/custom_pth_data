...
def load_dict(lang):
    """
        Read the dictionary file and return all words in it.
    """
    lang_dict = []

    # 경로 지정 수정됨
    dict_dir = os.path.dirname(os.path.abspath(__file__))
    dict_path = os.path.join(dict_dir, 'dicts', lang + '.txt')
...
def load_fonts(lang):
    ...
    if lang == 'cn':
        fonts_path = os.path.join(fonts_dir, 'fonts', 'cn')
    elif lang == 'ko':
        fonts_path = os.path.join(fonts_dir, 'fonts', 'ko')
    else:
        fonts_path = os.path.join(fonts_dir, 'fonts', 'latin')

    # 해당 폴더에서 폰트 목록 반환
    return [os.path.join(fonts_path, font) for font in os.listdir(fonts_path)]
